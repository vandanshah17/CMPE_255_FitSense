...
| 7 | Machine Learning Models | LR, RF, SVM, Gradient Boosting, implementations and configurations |
...
5. **Model Training:** Train four classifiers using stratified 5-fold cross-validation: Logistic Regression (baseline), Random Forest, SVM (RBF kernel), and Gradient Boosting (XGBoost/LightGBM). SMOTE is applied within each training fold.
6. **Hyperparameter Tuning:** Grid search over: LR regularization strength C; SVM C and gamma; RF max_depth and n_estimators; XGB learning_rate and max_depth.
...
Initial EDA is underway in `CMPE255_FitSense.ipynb`. Key analyses completed or in progress include:

- Distribution of the target variable `fit` across small, fit, and large classes (confirming significant class imbalance)
- Distributions of `height`, `weight`, and `age` broken down by fit label
- Correlation heatmap of all numerical features
- Breakdown of fit outcomes by `body type` and `category`
- Comparison of fit distributions between ModCloth and RentTheRunway sub-datasets

Findings from this section will be incorporated into the final report with supporting visualizations.

---

## 8. Machine Learning Experiments

The Logistic Regression baseline has been implemented. Results from Random Forest, SVM, and Gradient Boosting experiments are forthcoming. All comparative results will be reported in the final Results and Discussion section.

---

## 9. Results and Discussion

This section will present and compare all model evaluation metrics, confusion matrices, and feature importance rankings once experiments are complete. Error analysis will examine whether specific body types or product categories are systematically harder to predict.

---

## 10. Trend Analysis

This section will analyze systematic fit patterns across customer segments, including age groups, body types, and product categories, and will compare fit outcome distributions between the ModCloth and RentTheRunway platforms to identify any domain-specific shifts.
